...
def cbo_update_Euler_Maruyama(V, V_alpha, anisotropic, l, sigma, dt, device=None):
    ...
    with torch.no_grad():  # disables gradient calculation (reduces memory consumption - cannot use .backward() anymore)
        ...
        noise_weight = diff if anisotropic else torch.norm(diff, p=2, dim=1).reshape(-1,1)  # calculate norm (1xN - since norm on each row) or absolute value (Nxd - bc componentwise multiplication of (Nxd) and (Nxd))
...
def cbo_update_advanced_Heun(V, V_alpha, anisotropic, l, sigma, dt, function, alpha, device=None):
    ...
    with torch.no_grad():  # disables gradient calculation (reduces memory consumption - cannot use .backward() anymore)
        ...
        noise_weight_Euler = diff_Euler if anisotropic else torch.norm(diff_Euler, p=2, dim=1).reshape(-1,1)  # calculate norm (1xN - since norm on each row) or absolute value (Nxd - bc componentwise multiplication of (Nxd) and (Nxd)) for Euler step
        ...
        noise_weight_Heun = diff_Heun if anisotropic else torch.norm(diff_Heun, p=2, dim=1)  # calculate norm (1xN - since norm on each row) or absolute value (Nxd - bc componentwise multiplication of (Nxd) and (Nxd))
